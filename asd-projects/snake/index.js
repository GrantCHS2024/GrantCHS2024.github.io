...
setInterval(() => {
  ...
  console.log(timer);
  if(timer === 0){
    boardDIV.classList.remove("spin")
  }
  if(timer === -30){
      timer = 60;
      boardDIV.classList.add("spin");
...
document.addEventListener("keydown", (e) => {
  if (e.key === "a" && snake.head.direction !== "right") {
    snake.head.direction = "left";
  }
  else if(e.key === "d" && snake.head.direction !== "left") {
    snake.head.direction = "right";
  }
  else if(e.key === "w" && snake.head.direction !== "down") {
    snake.head.direction = "up";
  }
  else if(e.key === "s" && snake.head.direction !== "up") {
...
function endGame() {
  ...
  board.empty();
  boardDIV.classList.remove("spin");
  void boardDIV.offsetWidth; // Force reflow
  boardDIV.classList.add("spin");
  timer = 60;
